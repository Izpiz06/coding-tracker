  ...
  return (
    <main className="site-shell text-slate-100 flex items-center justify-center">
      <div className="w-full max-w-xl panel p-7">
        <h1 className="text-2xl font-bold mb-2 text-slate-100 tracking-tight">
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input disabled value={name} className="w-full input-dark p-2 opacity-75" />
            </div>
            <div>
              <label className="block text-xs uppercase tracking-wider text-slate-500 mb-1">Email</label>
              <input disabled value={email} className="w-full input-dark p-2 opacity-75" />
            </div>
          </div>

          <div>
            <label className="block text-xs uppercase tracking-wider text-slate-500 mb-1">GitHub Handle (optional)</label>
            <input
              value={githubHandle}
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider text-slate-500 mb-1">LeetCode Handle (required)</label>
            <input
          ...
          <div>
            <label className="block text-xs uppercase tracking-wider text-slate-500 mb-1">Codeforces Handle (required)</label>
            <input
        ...
        {message && (
          <div className={`mt-4 text-sm p-3 rounded-lg ${message.includes('saved') ? 'bg-emerald-900/30 border border-emerald-700/50 text-emerald-300' : 'bg-rose-900/30 border border-rose-700/50 text-rose-300'}`}>
            {message}
          </div>
        )}
      </div>
